...
/*!
 * Dark Side v6.2 — multi-source aggregator for Lampa
 * Changes from v6.1:
 * - Built-in test JSON URL for importing extended sources
 * - "Quick import" button in settings
 * 2025-08-13
 */
(function(){
  'use strict';
  ...
  function log(...a){ try{ console.log('[DarkSide v6.2]', ...a);}catch(e){} }
  ...
  async function loadExternalAdapters(){
    ...
    for(const url of list){ try{ await import(url); log('Loaded external adapter:', url); }catch(e){ log('Fail load', url, e); } }
  }
  ...
  function ensureMenuSource(){
    if(!hasLampa || !Lampa.Source) return;
    const src = {
    ...
    if(Lampa.Source.add) Lampa.Source.add(ID, src);
  }
  ...
  function registerSettings(){
    if(!hasLampa || !Lampa.Settings) return;
    const section = {
      title: 'Dark Side',
      items: [
        ...
        {title:'Импортировать список (JSON URL)', value:'Импорт', onClick(){ 
          const url = prompt('URL JSON', 'https://example.com/darkside_test_extended.json'); if(url) importFromJSON(url); 
        }},
        {title:'Быстрый импорт тестового списка', subtitle:'Загружает тестовый список расширенных источников', value:'Импорт', onClick(){ importFromJSON('https://example.com/darkside_test_extended.json'); }},
  ...
  function boot(){ if(!prefs.enabled) return; ensureMenuSource(); registerSettings(); log('Boot v6.2'); }
